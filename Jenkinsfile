    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists kyc_management;"
    ...
    stage('Generate docker image for development') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep kyc-management | grep latest | awk '{ print $3 }'`
    ...
    stage('Generate docker image for testing or production') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          images=`docker images | grep entropypool | grep kyc-management | grep $tag | awk '{ print $3 }'`
          for image in $images; do
            docker rmi $image -f
          done
        '''.stripIndent())
